module.exports = (io, socket) => {
	const changeSong = (payload) => {
		...
		socket.broadcast.to(payload.room).emit('broadcasted:song:change', payload.song);
	};
	const pauseSong = (payload) => {
		console.log(">>pauseSong - pausing song:" + JSON.stringify(payload));
		socket.broadcast.to(payload.room).emit('broadcasted:song:pause', payload.room);
	};
	const playSong = (payload) => {
		console.log(">>playSong - playing song:" + JSON.stringify(payload));
		socket.broadcast.to(payload.room).emit('broadcasted:song:play', payload.room);
	};
